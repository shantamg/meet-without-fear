...
class PrRequest:
    ...
    def title(self) -> str:
        return (
            f"loop: improve {self.stage_label} prompt for {self.moment_id} "
...
def create_alignment_pr(request: PrRequest, *, dry_run: bool = False) -> dict[str, Any]:
    ...
    if not branch:
        raise AlignmentLoopError("Refusing to open loop PR from a detached HEAD")
...
def build_pr_body(moment_id: str, run_dir: Path, score: dict[str, Any], improvement_plan: Path | None) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Gold comparison: `{display_path(run_dir / 'gold-comparison.md')}`",
            f"- AI response: `{display_path(run_dir / 'ai-response.md')}`",
            f"- Score rationale: `{display_path(run_dir / 'score-rationale.md')}`",
...
def reference_excerpt(moment: dict[str, Any]) -> tuple[str, str]:
    reference = str(moment.get("rubric", {}).get("reference_transcript_lines", ""))
    try:
        rel_path, line_range = reference.rsplit(":", 1)
        start_text, end_text = line_range.split("-", 1)
        start = int(start_text)
        end = int(end_text)
    except ValueError:
        return reference, "Reference transcript line range could not be parsed.\n"
    path = REPO_ROOT / rel_path
    if not path.exists():
        return reference, f"Reference transcript file not found: {display_path(path)}\n"
    lines = path.read_text(encoding="utf-8").splitlines()
    excerpt = "\n".join(f"{line_no}: {lines[line_no - 1]}" for line_no in range(start, min(end, len(lines)) + 1))
    return reference, excerpt + "\n"


def write_gold_comparison(run_dir: Path, moment: dict[str, Any], score: dict[str, Any]) -> Path:
    reference, excerpt = reference_excerpt(moment)
    ai_response = run_dir / "ai-response.md"
    score_rationale = run_dir / "score-rationale.md"
    body = [
        "# Gold Comparison",
        "",
        f"Moment: `{moment['id']}`",
        ...
        f"Verdict: `{score.get('verdict')}`",
        "",
        "## Gold Reference Excerpt",
        "",
        excerpt.rstrip(),
        "",
        "## AI Response Under Review",
        "",
        ai_response.read_text(encoding="utf-8").rstrip() if ai_response.exists() else "Missing ai-response.md.",
        "",
        "## Judge Rationale",
        "",
        score_rationale.read_text(encoding="utf-8").rstrip() if score_rationale.exists() else "Missing score-rationale.md.",
        "",
    ]
    path = run_dir / "gold-comparison.md"
    path.write_text("\n".join(body), encoding="utf-8")
    return path


def review_artifact_paths(run_dir: Path, version: Path, plan: Path, gold_comparison: Path) -> list[Path]:
    return [
        version,
        plan,
        run_dir / "patch-summary.md",
        gold_comparison,
        run_dir / "ai-response.md",
        run_dir / "score-rationale.md",
        run_dir / "judge-raw.json",
        run_dir / "score.json",
        run_dir / "seed-state.json",
        run_dir / "state-delta.json",
        run_dir / "run.json",
    ]
            ...
            if float(score.get("overall_score", 0)) < threshold:
                ...
                if args.dry_run:
                    ...
                    try:
                        ...
                        gold_comparison = write_gold_comparison(moment_run_dir, moment, score)
                        row["improvement_plan"] = display_path(plan)
                        row["gold_comparison"] = display_path(gold_comparison)
                        ...
                        pr_request = PrRequest(
                            ...
                            artifact_paths=review_artifact_paths(moment_run_dir, version, plan, gold_comparison),
