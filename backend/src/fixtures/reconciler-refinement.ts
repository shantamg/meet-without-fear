...
export const reconcilerRefinement: E2EFixture = {
  name: 'Reconciler Refinement Loop',
  description:
    'Partner journey where reconciler detects gaps (OFFER_SHARING), subject shares context, guesser refines, and reconciler re-runs. The hasContextAlreadyBeenShared guard intercepts the second reconciler run and marks as READY (PROCEED).',
...
`,
  ...
  operations: {
    // Reconciler analysis - finds SIGNIFICANT gaps on FIRST pass
    // On SECOND pass (after guesser resubmits), the hasContextAlreadyBeenShared guard
    // intercepts and marks as READY before this response would be used again.
    // However, the fixture system returns the same response for repeated operations,
    // which is fine - the guard handles the logic, not the fixture.
    'reconciler-analysis': {
      response: {
        alignment: {
          score: 45,
