...
> - `runReconciler()` — symmetric, runs for both directions simultaneously. **Currently unused.**
> - `runReconcilerForDirection()` — asymmetric, runs for one direction when partner completes Stage 1. **This is the active function.**
...
> **Dead Code Note:** The function `generateShareOffer()` exists in reconciler.ts but is
> never called. Share offers are generated by `generateShareSuggestion()` instead.
