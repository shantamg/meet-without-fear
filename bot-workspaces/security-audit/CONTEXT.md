# Security Audit — Workspace Context

## Purpose

Run a comprehensive 12-point security audit using parallel specialist agents. Report findings to #bot-ops and create GitHub issues for CRITICAL/HIGH findings.

## Stage Pointers

- `stages/1-scan/CONTEXT.md` — Parallel specialist agents
- `stages/2-report/CONTEXT.md` — Compile, prioritize, create issues, post

## Shared Resources Used

- `shared/references/credentials.md` — API access
- `shared/github/create-issue.md` — Issue creation for findings
- `shared/slack/slack-post.md` — Report to #bot-ops
- `shared/references/github-ops.md` — Issue labeling and assignment

## Key Docs to Read First

- `docs/architecture/system-overview.md`
- `docs/infrastructure/index.md`
- `docs/infrastructure/observability.md`
- `docs/infrastructure/production-access.md`
- `docs/architecture/assemblyai-asr.md`
- `docs/architecture/clerk-auth.md`
- `docs/architecture/ably-realtime.md`
