...
must_haves:
  ...
  artifacts:
    - path: "e2e/tests/two-browser-stage-0.spec.ts"
      provides: "Stage 0 regression test"
    - path: "e2e/tests/two-browser-stage-1.spec.ts"
      provides: "Stage 1 regression test"
  ...
  key_links:
    - from: "mobile/src/screens/UnifiedSessionScreen.tsx"
      to: "e2e/tests/two-browser-stage-0.spec.ts"
      via: "New Ably handlers fire during compact signing flow"
      pattern: "partner\\.signed_compact"
    - from: "mobile/src/screens/UnifiedSessionScreen.tsx"
      to: "e2e/tests/two-browser-stage-1.spec.ts"
      via: "New Ably handlers fire during feel-heard flow"
      pattern: "partner\\.advanced"
    - from: "mobile/src/screens/UnifiedSessionScreen.tsx"
...
<task type="auto">
  ...
  3. Run the Stage 0-1 E2E test:
     ```bash
     cd /Users/shantam/Software/meet-without-fear/e2e && npx playwright test --config=playwright.two-browser.config.ts two-browser-stage-0 two-browser-stage-1
     ```
     This test exercises:
  ...
  4. Run the Stage 2 E2E test:
     ...
     - Stage 2→3 transition (will now trigger updated `partner.stage_completed` handler)
