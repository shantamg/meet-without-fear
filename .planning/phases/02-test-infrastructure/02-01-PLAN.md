...
- `async createSession(): Promise<string>` - Creates a session via `POST /api/sessions` with `{ inviteName: config.userB.name }` using User A's E2E headers (including fixture ID). Parses the response JSON and extracts session ID and invitation ID from the data envelope using this exact pattern:
  ```typescript
...
2. Session creation uses `POST /api/sessions` with `{ inviteName: 'PartnerName' }` body and User A's E2E headers including fixture ID. Response format: `{ success: true, data: { session: { id }, invitation: { id } } }` (see single-user-journey.spec.ts line 88 for pattern). The extraction MUST be:
   ```typescript
   const responseData = await response.json();
   this.sessionId = responseData.data.session.id;
   this.invitationId = responseData.data.invitation.id;
   ```
   Do NOT use destructuring or any other pattern -- use these exact property paths so the code is unambiguous.
...
Run `cd /Users/shantam/Software/meet-without-fear/e2e && npx tsc --noEmit --project tsconfig.json 2>&1 | head -30` to verify TypeScript compilation. If no tsconfig.json exists in e2e/, run `npx tsc --noEmit e2e/helpers/two-browser-harness.ts --esModuleInterop --module commonjs --target es2020 --moduleResolution node --skipLibCheck` or verify by checking imports resolve correctly.
  ...
  <done>
`e2e/helpers/two-browser-harness.ts` exists with TwoBrowserHarness class (setupUserA, setupUserB, createSession, acceptInvitation, navigateUserA, navigateUserB, teardown, cleanup methods) and waitForPartnerUpdate function. createSession extracts IDs via `responseData.data.session.id` and `responseData.data.invitation.id`. Class exposes sessionId, invitationId, userAId, userBId, userAPage, userBPage, config. File compiles without TypeScript errors.
  </done>
</task>
...
8. createSession extracts IDs via `responseData.data.session.id` and `responseData.data.invitation.id`
...
- createSession extracts and stores sessionId via `responseData.data.session.id` and invitationId via `responseData.data.invitation.id`
