...
must_haves:
  truths:
    ...
    - "Tests navigate full UI from scratch (API session creation via harness is OK; no SessionBuilder stage-skipping)"
...
3. Accept invitation via harness: `await harness.acceptInvitation()`. Do NOT make a manual API call here -- the harness method handles the `POST /api/invitations/{invitationId}/accept` with User B's headers internally.
4. Navigate User A to session: `await harness.navigateUserA()`.
5. User A signs compact: `await signCompact(harness.userAPage)`.
6. User A handles mood check: `await handleMoodCheck(harness.userAPage)`.
7. User A sees chat input: `await expect(harness.userAPage.getByTestId('chat-input')).toBeVisible()`.
8. Navigate User B to session: `await harness.navigateUserB()`.
9. User B signs compact: `await signCompact(harness.userBPage)`.
10. User B handles mood check: `await handleMoodCheck(harness.userBPage)`.
11. User B sees chat input: `await expect(harness.userBPage.getByTestId('chat-input')).toBeVisible()`.
12. **Verify User A sees partner name:** `await expect(harness.userAPage.getByTestId('session-chat-header-partner-name')).toHaveText('Bob')` (with timeout 15000 - Ably update may be needed). If flaky, use `waitForPartnerUpdate()` with reload fallback.
13. **Verify User B sees partner name:** `await expect(harness.userBPage.getByTestId('session-chat-header-partner-name')).toHaveText('Alice')` (with timeout 15000).
...
- Full UI navigation works (no SessionBuilder stage-skipping)
...
- User A creates session with `inviteName: 'Bob'`. User B accepts invitation via `harness.acceptInvitation()`.
...
- Do NOT make manual API calls for invitation acceptance -- always use `harness.acceptInvitation()`.
  </action>
  <verify>
...
The test should pass with both users connecting, signing compact, seeing each other's partner name, and receiving different fixture-based AI responses.
  ...
  <done>
Smoke test passes: Both users connect to same session via real Ably, navigate full UI (compact + mood check), see each other's partner name, and receive different AI responses from per-user fixtures. No SessionBuilder seeding used. Invitation accepted via `harness.acceptInvitation()` (not manual API call).
  </done>
</task>
...
6. Smoke test calls `harness.acceptInvitation()` instead of manual API calls
</verification>
